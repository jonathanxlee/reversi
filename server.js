...
io.sockets.on('connection', function (socket) {
   ...
   socket.on('play_token', function(payload) {
        ...
        /* If the current attempt at playing the token is out of turn then erropr */ 
        ...
        if(
            ...
            var error_message = 'play_token played by the wrong player';
            log(error_message);
            socket.emit('play_token_response',{
                                                result: 'fail',
                                                message: error_message
...
/*
 * Check of the position at r,c contains the opposite of 'who' on the board
 * and of the line dincated by adding dr to r and dc to c eventually ends in the 
 * who color.
 */
...
function send_game_update(socket, game_id, message){
    ...
    do{
        ...
        if(numClients > 2){
            ...
            if(games[game_id].player_white == roomObject.sockets[0]){
                games[game_id].player_white.username = '';
                games[game_id].player_white.socket = '';

            }
            if(games[game_id].player_black == roomObject.sockets[0]){
